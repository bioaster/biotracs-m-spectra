...
if isscalar(N) && rem(N,1)==0
    ...
    % D. A. Ouattara, Bioaster 2015
    % Change native-matlab code
    % Allow negative index (for negative chemical shift in NMR signals)
    %R = [max(0,min(X)) max(X)];
    R = [min(X) max(X)];        
...
if cutoffGiven
    ...
    % D. A. Ouattara, Bioaster 2015
    % Change native-matlab code  
    % Bug-fix : NMR chemical shift < 50, so the  predifined Xmin = 50 would
    % fail in this case
    %startAt = floor(min(N/5,find(XOUT>Xmin,1,'first')));
    founded = find(XOUT>Xmin,1,'first');
    if ~isempty(founded)
        startAt = floor(min(N/5,founded)); 
    else
        startAt = floor(N/5);
    end
    
    
%     find(XOUT>Xmin,1,'first') 
%     startAt
%     N
%     (startAt:N)/N
%     df(startAt:N)
%     polOrder  
%     pfit = polyfit((startAt:N)/N,df(startAt:N),polOrder)
%     pval = max(polyval(pfit,(startAt:N)/N))
...
if DF >= 1  % it is UPSAMPLE, then just interpolate
    % Change native-matlab code here
    % Spline interplation may create new peaks that are artefacts in NMR or
    % MS spectra
    % D. A. Ouattara, BIOASTER 2016
    %YOUT = interp1(X,Y,XOUT,'spline',0);  
    YOUT = interp1(X,Y,XOUT,'pchip',0);  
