                    ...
                    if parser.isStartElement()
                        ...
                        switch childElemNm %char(parser.getLocalName())
                            case {'nameValue','processingOperation'}
                                ...
                                % BugFix: processingOperation ignored !
                                %continue;
                            ...
                            case 'comment'
                                ...
                                % D.A. Ouattara, BIOASTER
                                % BugFix: count2 is not defined !
                                %out.mzXML.msRun.(elementName)(count+1).(childElemNm)(count2+1).value = char(parser.getElementText());
                                out.mzXML.msRun.(elementName)(count+1).(childElemNm)(1).value = char(parser.getElementText());
...
% Changed Here
% D. A. Ouattara
% Some converted mzXML files do not not fullfill original specifications
% Try to handle these files
try
    %remove empty structures from scan struct array
    out.scan = out.scan([out.scan.num]);
catch
    disp('Cannot remove empty structures from scan struct array. Probably ms-ms data file' );
end
        ...
        switch childElemNmScan 
            ...
            case 'precursorMz'   
                ...
                try
                    ...
                    %out.scan
                    %out.(elementNameScan)
                    out.(elementNameScan)(countScan+1)
                    
                    out.(elementNameScan)(countScan)
                end
            ...
            case 'scan'
                ...
                if countScan > mzinfo.NumberOfScans
                    break;
                end
...
switch endian
    case 'L'
        %mzpeaks = swapbytes(typecast(peaks,'double'));      %because several functions does not operate with singles
        if precision == 32
