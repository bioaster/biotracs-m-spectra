...
if flag % a Reference signal has been chosen
    ...
    intdif = diff(sinter);
    if any(intdif(2:2:length(intdif)) < 0)
        %D.A. Ouattara, BIOASTER 23 Mar. 2017
        %uiwait(msgbox('The user-defined intervals are overlapping: is that intentional?','Warning','warn'));
        warning('The user-defined intervals are overlapping. Please, ensure that it is intentional.');
    end
    ...
    if flag_coshift
        ...
        if flag
            ...
            if ~whole
                ...
                title('Whole spectra aligned data');
            end
...
if flag_coshift
    ind = ind + wint * ones(1,size(ind,2)); 
end
...
if isempty(flag) || (numel(flag) == 1 && flag)
    flag = isnan(X);
    N    = true;
elseif isequal(size(flag),size(X))
    N = true;
elseif numel(flag) == 1 && ~flag
    N = false;
else
    error('Invalid flag'),
end
...
if ppm(1) < ppm(2), I = scal2pts(ppm(1) + d,ppm,varargin{:}) - 1;
    ...
    I = length(ppm) - scal2pts(ppm(end) + d,ppm,varargin{:}) + 1;
end
